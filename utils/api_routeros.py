...
class Api:
    ...
    def __init__(self, address, user=USER, password=PASSWORD, use_ssl=True, port=PORT,
    ...
    def login(self):
        sentence = ['/auth', '=name=' + self.user, '=password=' + self.password]
        reply = self.communicate(sentence)
        if len(reply[0]) == 1 and reply[0][0] == '!done':
            # If auth process was successful
            self.log('Logged in successfully!')
            return reply
        elif 'Error' in reply:
            # Else if there was some kind of error during auth process
            self.log('Error in auth process - {}'.format(reply))
            raise LoginError('Login ' + reply)
        elif len(reply[0]) == 2 and reply[0][1][0:5] == '=ret=':
            # Else if RouterOS uses old API auth method, code continues with old method
            self.log('Using old auth process.')
            md5 = hashlib.md5(('\x00' + self.password).encode('utf-8'))
            md5.update(binascii.unhexlify(reply[0][1][5:]))
            sentence = ['/auth', '=name=' + self.user, '=response=00'
